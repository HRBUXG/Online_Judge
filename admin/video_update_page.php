...
<div class="container">
    ...
    if (isset($_GET['videoinformation'])) {
        ...
        $sql = "select video_id,video_name,video_describe,video_framer,video_upload_time,video_source,video_privilege
			...
			like ?";
        $res = pdo_query($sql, $videoinformation);
        /*$rows=mysql_affected_rows($conn);//获取行数
        $colums=mysql_num_fields($res);//获取列数*/
        //echo "jol数据库的".$table_name."表的所有用户数据如下：<br/>";
        //echo "共计".$rows."行 ".$colums."列<br/>";
    } else {
        $sql = "select video_id,video_name,video_describe,video_framer,video_upload_time,video_source,video_privilege from video";
        $res = pdo_query($sql);
        /*$rows = mysql_affected_rows($conn);//获取行数
        $colums = mysql_num_fields($res);//获取列数*/
        //echo "jol数据库的".$table_name."表的所有用户数据如下：<br/>";
    ...
    <table id='problemset' width='90%' class='table table-striped'>
        ...
        foreach ($res as $row) {
            ...
            echo "<td class='hidden-xs' >" . $row['video_privilege'] . "</td>";
            ?>
            <td class='hidden-xs'>
