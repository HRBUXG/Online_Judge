...
if($keyword) {
	$keyword="%$keyword%";
	$sql="select `problem_id`,`title`,`accepted`,`in_date`,`source`,`defunct` FROM `problem` where title like ? or source like ?";
	$result=pdo_query($sql,$keyword,$keyword);
}else{
	$sql="select `problem_id`,`title`,`accepted`,`in_date`,`source`,`defunct` FROM `problem` where problem_id>=? and problem_id<=? order by `problem_id` ASC";
	$result=pdo_query($sql,$pstart,$pend);
}
?>
<form action=problem_list.php method="post" >
<input name=keyword><input class="btn btn-primary btn-sm" type=submit value="<?php echo $MSG_SEARCH?>" ></form>
...
<input type=submit name='problem2contest' value='CheckToNewContest' style="background-color: lightblue">
<input type=submit name='enable' value='Batch Open Access' style="background-color: lightblue" onclick='$("form").attr("action","problem_df_change.php")'>
<input type=submit name='disable' value='Batch Reserve' style="background-color: lightblue"  onclick='$("form").attr("action","problem_df_change.php")'>
<tr id="tableTitle"><td>PID<td>Title<td>AC<td>Date

<?php
if(isset($_SESSION[$OJ_NAME.'_'.'administrator'])||isset($_SESSION[$OJ_NAME.'_'.'problem_editor'])){
        if(isset($_SESSION[$OJ_NAME.'_'.'administrator'])||isset($_SESSION[$OJ_NAME.'_'.'problem_editor']))   
	//echo "<td>Status<td>Delete";
        echo "<td>Source<td>Status<td>Delete";
	echo "<td>Edit<td>TestData</tr>";
}
foreach($result as $row){
        echo "<tr onmouseover='over_color(this)' onmouseout='remove_color(this)' onclick='click_color(this)' class='tr-color cursor'>";
