...
<div class="container" style="width:100%">
    ...
    <div id="header" style="padding:5px;background-color:#000; color:#FFF;text-align:center;">
        <h2><?php echo $MSG_UPDATE . $MSG_NOW_PLAYINFORMATION ?></h2>
    </div>
    <br>

    <form action="update_now_playinformation_page.php" class="center">
    ...
    if (isset($_GET['playinformation'])) {
        ...
        $sql = "select user_id,username,grade,email,professional,college,work 
		    from now_players where concat(user_id,username,grade,professional,college) like ?";
        $res = pdo_query($sql, $playinformation);
        //echo "jol数据库的".$table_name."表的所有用户数据如下：<br/>";
        //echo "共计".$rows."行 ".$colums."列<br/>";
    } else {
        $sql = "select user_id,username,grade,email,professional,college,work from now_players";
        $res = pdo_query($sql);
        //echo "jol数据库的".$table_name."表的所有用户数据如下：<br/>";
        //echo "共计".$rows."行 ".$colums."列<br/>";
    }
    ...
    <div id="update_former_players">
        <table border="1" class='table table-striped' width="100%" style="word-wrap:break-word;word-break:break-all;">
            ...
            {
            echo "<tr>";
                echo "<td class='hidden-xs' >".$row['user_id']."</td>";
                echo "<td class='hidden-xs' >".$row['username']."</td>";
                echo "<td class='hidden-xs' >".$row['grade']."</td>";
                echo "<td class='hidden-xs' >".$row['email']."</td>";
                echo "<td class='hidden-xs' >".$row['professional']."</td>";
                echo "<td class='hidden-xs' >".$row['college']."</td>";
                echo "<td class='hidden-xs' >".$row['work']."</td>";
            ?>
            <td class='hidden-xs'>
...
<script>
    $('.del').click(function () {
        ...
        window.location.href = 'update_now_playinformation.php?flagdel=' + flagdel + '&team_member_id=' + team_member_id;
    ...
    $('.upd').click(function () {
        ...
        window.location.href = 'update_now_playinformation.php?team_member_id=' + team_member_id + '&flagupdate=' + flagupdate;
    });
