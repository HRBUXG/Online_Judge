...
Blockly.Workspace.prototype.deleteVariable = function (name) {
    ...
    for (var i = 0, block; block = uses[i]; i++) {
        if (block.type == 'procedures_defnoreturn' ||
            ...
            Blockly.alert(
                Blockly.Msg.CANNOT_DELETE_VARIABLE_PROCEDURE.replace('%1', name).replace('%2', procedureName));
            return;
        }
    }
    ...
    if (uses.length > 1) {
        ...
        Blockly.confirm(
            Blockly.Msg.DELETE_VARIABLE_CONFIRMATION.replace('%1', uses.length).replace('%2', name),
            function (ok) {
                if (ok) {
                    doDeletion();
                }
