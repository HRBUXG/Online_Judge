...
//	   echo "<br/>";
        ...
        if (is_uploaded_file($_FILES['upfile']['tmp_name'])) {
            ...
            if (move_uploaded_file($_FILES['upfile']['tmp_name'], $toFileName)) {
                ...
                $sql = "insert into file (file_name,file_address,file_describe,file_upload_time,file_framer,file_source,file_privilege) values(?,?,?,?,?,?,?,?,?)";
                $result = pdo_query($sql,$fname,"",$faddress,$fdescribe,$fupload_time,$fframer,$fsource,"",$fprivilege);
                echo $result;
                if ($result) {
                    echo "<script>
				         alert('文件已添加到数据表中')
