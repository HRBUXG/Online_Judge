...
<?php      
     //取得名字开始
     $filename = $_FILES["fps"]["name"];
     $namestr = split("-","$filename");
     $namestr = $namestr[1];
     //取得名字结束
?>
...
//		$test_output = getValue ( $searchNode, 'test_output' );
		...
		if(!hasProblem($title )){
			...
			//添加权限开始
			            $ipid = "i$pid";
			            $ppid = "p$pid";
			            $sql = "DELETE FROM privilege where `"."rightstr`=".'"'.$ppid.'"';
			            pdo_query($sql);
			            $sql = "select * from privilege where `"."rightstr`=".'"'.$ipid.'"';
			            $result1 = pdo_query($sql);
			            if(count($result1)==0){
							$sql='insert into privilege values("'.$GLOBALS['namestr'].'","'.$ipid.'","N");';
						}
			            else{
							$sql='update privilege set `'."user_id`=".'"'.$GLOBALS['namestr'].'"'." where `"."rightstr`=".'"'.$ipid.'"';
						}
			            pdo_query($sql);
			//添加权限结束
			mkdir ( $basedir );
