...
if (isset($_GET['cid'])) {
    ...
    foreach ($result as $row) {
        ...
        $view_problemset[$cnt][1] = "Problem &nbsp;" . $PID[$cnt];//$row['pid']." Problem &nbsp;".$PID[$cnt];
...
//  echo "$mycontests";
    ...
    if ($keyword) {
        $sql = "select *  from contest left join (select * from privilege where rightstr like 'm%') p on concat('m',contest_id)=rightstr where contest.defunct='N' and contest.title like ? $wheremy  order by start_time desc ";

        $sql .= " limit " . strval($pstart) . "," . strval($pend);
        $result = pdo_query($sql, $keyword);
    } else {
        $sql = "select *  from contest left join (select * from privilege where rightstr like 'm%') p on concat('m',contest_id)=rightstr where contest.defunct='N' $wheremy  order by start_time desc ";
        $sql .= " limit " . strval($pstart) . "," . strval($pend);
    ...
    foreach ($result as $row) {
        ...
        if ($now > $end_time) {
            ...
            $view_contest[$i][2] = "<span>" . $row['start_time'] . "</span>&nbsp;";
            $view_contest[$i][3] = "<span class=blue style='color:rgb(0,128,0);font-weight:bold;'>$MSG_Start</span>";
            $view_contest[$i][2] .= "<span class=green style='color:green'>$MSG_TotalTime&nbsp;" . formatTimeLength($length) . "</span>";
            // running

        } else {
            $view_contest[$i][2] = "<span>" . $row['start_time'] . "</span>&nbsp;";
            $view_contest[$i][3] = "<span class=blue style='color:blue;font-weight:bold'>$MSG_Running</span>";
            $view_contest[$i][2] .= "<span class=green style='color:rgb(255,165,0)'> $MSG_LeftTime " . formatTimeLength($left) . " </span>";
        }
