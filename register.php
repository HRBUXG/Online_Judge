...
if ($grade != $regrade) {
    $err_str = $err_str . "grade do not march the userid!\\n";
    $err_cnt++;
}
if ($class != $reclass) {
    $err_str = $err_str . "class do not march the userid!\\n";
    $err_cnt++;
}
...
if ($len == 11 || $len == 0) {
    ...
    $err_str = $err_str . "phonenumber is not 11 digit!\\n";
    $err_cnt++;
}
if (!filter_var($email, FILTER_VALIDATE_EMAIL)) {
    $err_str = $err_str . "非法邮箱格式\\n";
    $err_cnt++;
}
if ($err_cnt > 0) {
...
$sql = "INSERT INTO `users`("
    . "`user_id`,`email`,`ip`,`accesstime`,`password`,`reg_time`,`nick`,`school`,`defunct`,`grade`,`class`,`phonenumber`)"
    . "VALUES(?,?,?,NOW(),?,NOW(),?,?,?,?,?,?)";
$rows = pdo_query($sql, $user_id, $email, $ip, $password, $nick, $school, $defunct, $grade, $class, $phonenumber);
// or die("Insert Error!\n");
