...
<p><font color=green>Q</font>:채점을 위해 사용되는 컴파일러와 각 컴파일러의 옵션은 어떤것입니까?<br>
    <font color=red>A</font>:채점 시스템은 <a href="http://www.debian.org/">Debian Linux</a>에서 구동됩니다. 현재 C/C++ 코드 컴파일을 위해 <a
            href="http://gcc.gnu.org/">GNU GCC/G++</a>를 사용하고 있으며, pascal 코드 컴파일을 위해 <a href="http://www.freepascal.org">Free
        Pascal</a> 을, 그리고 Java 코드 컴파일을 위해 <a
            href="http://www.oracle.com/technetwork/java/index.html">sun-java-jdk1.6</a>를 사용합니다. 컴파일을 위해서 다음과 같은 옵션을
    사용합니다:<br>
</p>
<table border="1">
    <tr>
...
<font color=red>A</font>:채점을 위해서 입력은 stdin('Standard Input')을 통해 받게 되며, stdout('Standard Output')에 출력하게 됩니다. 자세하게 이야기 하자면, 입력을 위해서는 'scanf(C)/cin(C++)'을, 출력을 위해서 'printf(C)/cout(C++)' 을 사용하게 됩니다
<br>
...
<ul>
    <li>G++에선 <font color=blue>main</font>이 반드시 <font color=blue>int</font>형으로 선언되어야 하며, <font color=blue>void
            main</font>을 사용하게 되면 Compile Error를 받게 됩니다..<br>
    <li><font color=blue>for</font>(<font color=blue>int</font> <font color=green>i</font>=0...){...}"와 같이 for문 안에
        int변수를 선언하였을 경우 block을 벗어났을 때 i 변수는 사라지게 됩니다.<br>
    <li><font color=green>itoa</font> 는 ANSI 에서 규정한 표준 함수가 아닙니다.<br>
    <li>VC에서의 <font color=green>__int64</font>는 ANSI 표준이 아닙니다. 하지만 64비트 integer 변수를 사용하기 위해 <font color=blue>long
            long</font>을 사용할 수 있습니다.<br>
</ul>
<hr>
...
<p><font color=blue>Accepted</font> : 제출한 프로그램이 모든 테스트 데이터를 통과했음을 뜻합니다.<br>
    ...
    <br>
    <font color=blue>Output Limit Exceeded</font>: 예상하는 보다 많은 출력이 발생한 경우 입니다. 일반적으로 프로그램이 무한 루프에 빠졌을 경우에 일어납니다. 현재 채점
    시스템에서 출력 제한은 1메가 바이트로 제한됩니다.<br>
    <br>
    <font color=blue>Runtime Error</font> : 실행 도중에 'segmentation fault','floating point exception','used forbidden
    functions', 'tried to access forbidden memories' 등의 에러가 발생하여서 실행도중에 프로그램이 종료된 경우 입니다.<br>
</p>
<p><font color=blue>Compile Error</font> : 컴파일러가 제출한 소스코드를 컴파일 하지 못한 경우입니다. 물론 경고 메시지(warning message)는 에러 메시지로 간주하지
    않습니다. 채점 결과를 클릭하면 실제 에러 메시지를 볼 수 있습니다.<br>
    <br>
</p>
<hr>
...
<center>
    <table width=100% border=0>
        <tr>
            <td align=right width=65%><a href="index.php"><font color=red><?php echo $OJ_NAME ?></font></a> <a
                        href=""><font color=red>V1.9.9.0</font></a></td>
        </tr>
    </table>
</center>
