...
<div class="container">
    ...
    if ($flagdel == "del") {
        //echo "进行删除操作:</br>";
        //echo "name: ".$name."</br>";
        $sql = "select picture from now_players where user_id=?";
        $data = pdo_query($sql, $id);
        $address = $data[0]['picture'];
        echo "address:" . $address;

        $delete_former_players = "delete from now_players where user_id=?";
    ...
    if ($flagupdate == "upd") {
        //echo "123";
        $sql_info="select username,grade,email,professional,college,work,awards,introduce,post,picture from now_players where user_id=?";
        ...
        <form method=POST
              action="update_now_playinformation.php?flagup=up&team_member_id=<?php echo $id; ?>&oddaddress=<?php echo $picture; ?>"
    ...
    if ($flagup == "up") {
        ...
        $sql11 =
		  ...
		  username=?,user_id=?,post=?,grade=?,email=?
		   ...
		   ,work=?,awards=?,introduce=?
	       where user_id=?";
        $res11 = pdo_query($sql11, $u_uname, $u_id, $u_post, $u_grade, $u_email, $u_professional, $u_college
            , $u_work, $new_u_awards, $new_u_introduce, $id);
        if ($res11)
        {
        ?>
            <script>
    ...
    if ($flagadd == "add") {
       ...
       print_r($_FILES);*/
        ...
        if ($_FILES['upfile']['error'] > 0) {
            ...
            echo "u_awards:".$u_awards."<br>";
            echo "u_introduce:".$u_introduce."<br>";*/
            $sql10 = "insert into now_players value(?,?,?,?,?,?,?,?,?,?,?)";
            $res10 = pdo_query($sql10, $u_id, $u_email, $u_grade, $u_picture, $u_professional, $u_uname, $new_u_awards, $new_u_introduce
                , $u_work, $u_college, $u_post);
        if ($res10) {
            ?>
            <script>
                alert("添加成功");
        ...
        /* echo "dirName:".$dirName."<br>";
         ...
         echo "u_awards:".$u_awards."<br>";
         echo "u_introduce:".$u_introduce."<br>";*/
        $sql11 = "insert into now_players value(?,?,?,?,?,?,?,?,?,?,?)";
        $res11 = mysql_query($sql11, $u_id, $u_email, $u_grade, "", $u_professional, $u_uname, $new_u_awards, $new_u_introduce, $u_work, $u_college,$u_post);
        if ($res11)
        {
        ?>
            <script>
