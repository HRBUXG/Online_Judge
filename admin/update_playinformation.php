...
if($flagdel=="del")
{
	//echo "进行删除操作:</br>";
	//echo "name: ".$name."</br>";
	$sql="select picture from former_players where user_id='$id'";  
    $data=mysql_query($sql,$conn); 
    $address=mysql_result($data,0);
	echo "address:".$address;
	
	$delete_former_players="delete from former_players where user_id='$id'";
...
if($flagupdate=="upd")
{
   ...
   //邮箱
   $select_email="select email from former_players where user_id='$id'";  
   $select_email_res=mysql_query($select_email,$conn); 
   $email=mysql_result($select_email_res,0);
   //专业
   $select_professional="select professional from former_players where user_id='$id'";  
...
?>
  <h2 align="center" style="color:#F00"> 修改<?php echo $uname?>的信息： </h2>
  <form method=POST  action="update_playinformation.php?flagup=up&team_member_id=<?php echo $id;?>&oddaddress=<?php echo $picture;?>" 
