...
if(!empty($_FILES['upfile']['name']))
{
	...
	else
    {
	   ...
	   /*插入数据库*/
	   $link_add_video=mysql_connect("localhost","root","HRBUXGOJ");
       if(!$link_add_video)
       {  
             echo "连接失败";  
       }  
       mysql_select_db("jol",$link_add_video);
	   ...
	   //echo "new_vname:";
	   //echo $new_vname; 
	   $select_samename="select video_name from video where video_name='$vname'";  
       $select_samename_res=mysql_query($select_samename,$link_add_video); //判断是否有一样的名字的数据
	   $namename=mysql_result($select_samename_res,0);
	   ...
	   if(!$namename)
	   {
	       ...
	       values
		   ('$vname','$vdescribe','$vsource','$vaddress','NULL','$vframer','$vupload_time','NULL','$vprivilege')";
	   $sql_insert_result=mysql_query($sql_insert,$link_add_video);
