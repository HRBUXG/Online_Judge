...
for ($i = 0; $i < $rows_cnt; $i++) {
    ...
    $view_mystatus[$i][0] = $row['solution_id'];

    if ($row['contest_id'] > 0) {
//                $view_mystatus[$i][1]= "<a href='contestrank.php?cid=".$row['contest_id']."&user_id=".$row['user_id']."#".$row['user_id']."'>".$row['user_id']."</a>";
        if (isset($_SESSION[$OJ_NAME . '_' . 'administrator']))
            $view_mystatus[$i][1] = "<a href='contestrank.php?cid=" . $row['contest_id'] . "&user_id=" . $row['user_id'] . "#" . $row['user_id'] . "' title='" . $row['ip'] . "'>" . $row['user_id'] . "</a>";
        else
            $view_mystatus[$i][1] = "<a href='contestrank.php?cid=" . $row['contest_id'] . "&user_id=" . $row['user_id'] . "#" . $row['user_id'] . "'>" . $row['user_id'] . "</a>";
    } else {
        //        $view_mystatus[$i][1]= "<a href='userinfo.php?user=".$row['user_id']."'>".$row['user_id']."</a>";
        if (isset($_SESSION[$OJ_NAME . '_' . 'administrator']))
            $view_mystatus[$i][1] = "<a href='userinfo.php?user=" . $row['user_id'] . "' title='" . $row['ip'] . "'>" . $row['user_id'] . "</a>";
        else
            $view_mystatus[$i][1] = "<a href='userinfo.php?user=" . $row['user_id'] . "'>" . $row['user_id'] . "</a>";
    }

    if ($row['contest_id'] > 0) {
        $view_mystatus[$i][2] = "<div class=center><a href='problem.php?cid=" . $row['contest_id'] . "&pid=" . $row['num'] . "'>";
        if (isset($cid)) {
            $view_mystatus[$i][2] .= $PID[$row['num']];
        } else {
            $view_mystatus[$i][2] .= $row['problem_id'];
        }
        $view_mystatus[$i][2] .= "</div></a>";
    } else {
        $view_mystatus[$i][2] = "<div class=center><a href='problem.php?id=" . $row['problem_id'] . "'>" . $row['problem_id'] . "</a></div>";
    }
    switch ($row['result']) {
        case 4:
    ...
    $view_mystatus[$i][3] = "<span class='hidden' style='display:none' result='" . $row['result'] . "' ></span>";
    if (intval($row['result']) == 11 && ((isset($_SESSION[$OJ_NAME . '_' . 'user_id']) && $row['user_id'] == $_SESSION[$OJ_NAME . '_' . 'user_id']) || isset($_SESSION[$OJ_NAME . '_' . 'source_browser']))) {
        $view_mystatus[$i][3] .= "<a href='ceinfo.php?sid=" . $row['solution_id'] . "' class='" . $judge_color[$row['result']] . "'  title='$MSG_Tips'>" . $MSG_Compile_Error . "";

        if ($row['result'] != 4 && isset($row['pass_rate']) && $row['pass_rate'] > 0 && $row['pass_rate'] < .98)
            $view_mystatus[$i][3] .= (100 - $row['pass_rate'] * 100) . "%</a>";
        else
            $view_mystatus[$i][3] .= "</a>";
        ...
        $view_mystatus[$i][3] .= "<a href='reinfo.php?sid=" . $row['solution_id']
        ...
        if (!$lock || $lock_time > $row['in_date'] || $row['user_id'] == $_SESSION[$OJ_NAME . '_' . 'user_id']) {
            if ($OJ_SIM && $row['sim'] > 80 && $row['sim_s_id'] != $row['s_id']) {
                $view_mystatus[$i][3] .= "<span class='" . $judge_color[$row['result']] . "'  title='$MSG_Tips'>*" . $judge_result[$row['result']] . "";
                if ($row['result'] != 4 && isset($row['pass_rate']) && $row['pass_rate'] > 0 && $row['pass_rate'] < .98)
                    $view_mystatus[$i][3] .= (100 - $row['pass_rate'] * 100) . "%</span>";
                else
                    $view_mystatus[$i][3] .= "</span>";

                if (isset($_SESSION[$OJ_NAME . '_' . 'source_browser'])) {

                    $view_mystatus[$i][3] .= "<a href=comparesource.php?left=" . $row['sim_s_id'] . "&right=" . $row['solution_id'] . "  class='btn-info'  target=original>" . $row['sim_s_id'] . "(" . $row['sim'] . "%)</a>";
                } else {

                    $view_mystatus[$i][3] .= "<span class='btn-info'>" . $row['sim_s_id'] . "</span>";

                }
                if (isset($_GET['showsim']) && isset($row['sim_s_id'])) {
                    $view_mystatus[$i][3] .= "<span sid='" . $row['sim_s_id'] . "' class='original'></span>";
    ...
    if (isset($_SESSION[$OJ_NAME . '_' . 'http_judge'])) {
        $view_mystatus[$i][3] .= "<form class='http_judge_form form-inline' >
					<input type=hidden name=sid value='" . $row['solution_id'] . "'>";
        $view_mystatus[$i][3] .= "</form>";
    }
    ...
    if ($flag) {
        ...
        if ($row['result'] >= 4) {
            $view_mystatus[$i][4] = "<div id=center class=red>" . $row['memory'] . "</div>";
            $view_mystatus[$i][5] = "<div id=center class=red>" . $row['time'] . "</div>";
            //echo "=========".$row['memory']."========";
        } else {
            $view_mystatus[$i][4] = "---";
            $view_mystatus[$i][5] = "---";
        ...
        if (!(isset($_SESSION[$OJ_NAME . '_' . 'user_id']) && strtolower($row['user_id']) == strtolower($_SESSION[$OJ_NAME . '_' . 'user_id']) || isset($_SESSION[$OJ_NAME . '_' . 'source_browser']))) {
            $view_mystatus[$i][6] = $language_name[$row['language']];
        } else {

            $view_mystatus[$i][6] = "<a target=_blank href=showsource.php?id=" . $row['solution_id'] . ">" . $language_name[$row['language']] . "</a>";
            if ($row["problem_id"] > 0) {
                if ($row['contest_id'] > 0) {
                    $view_mystatus[$i][6] .= "/<a target=_self href=\"submitpage.php?cid=" . $row['contest_id'] . "&pid=" . $row['num'] . "&sid=" . $row['solution_id'] . "\">Edit</a>";
                } else {
                    $view_mystatus[$i][6] .= "/<a target=_self href=\"submitpage.php?id=" . $row['problem_id'] . "&sid=" . $row['solution_id'] . "\">Edit</a>";
                }
            }
        }
        $view_mystatus[$i][7] = $row['code_length'] . " B";
    ...
    $view_mystatus[$i][8] = $row['in_date'];
    $view_mystatus[$i][9] = $row['judger'];
...
/////////////////////////Template
if (isset($_GET['cid']))
    require("template/" . $OJ_TEMPLATE . "/contestmystatus.php");
else
    require("template/" . $OJ_TEMPLATE . "/mystatus.php");
/////////////////////////Common foot
